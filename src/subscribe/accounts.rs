...
pub fn program_id(network: &Network) -> Pubkey {
    match network {
        Network::Mainnet => {
            Pubkey::from_str("9ExbZjAapQww1vfcisDmrngPinHTEfpjYRWMunJgcKaA")
                .expect("Invalid mainnet program_id")
        }
        Network::Devnet => {
            Pubkey::from_str("6pW64gN1s2uqjHkn1unFeEjAwJkPGHoppGvS715wyP2J")
...
pub fn token_mint(network: &Network) -> Pubkey {
    match network {
        Network::Mainnet => {
            Pubkey::from_str("Zhw9TVKp68a1QrftncMSd6ELXKDtpVMNuMGr1jNwdeL")
                .expect("Invalid mainnet token_mint")
        }
        Network::Devnet => {
            Pubkey::from_str("4Zao8ocPhmMgq7PdsYWyxvqySMGx7xb9cMftPMkEokRG")
